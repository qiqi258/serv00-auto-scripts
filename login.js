...
(async () => {
    ...
    for (const account of accounts) {
        ...
        try {
            ...
            if (isLoggedIn) {
                const 现在Utc = formatToISO(new Date());
                const 现在Beijing = formatToISO(new Date(new Date()。getTime() + 8 * 60 * 60 * 1000)); // 北京时间东8区
                console。log(`账号 ${username} 于北京时间 ${现在Beijing}（UTC时间 ${现在Utc}）登录成功！`);
                if (telegramToken && telegramChatId) {
                    await sendTelegramMessage(telegramToken， telegramChatId， `账号 ${username} 于北京时间 ${现在Beijing}（UTC时间 ${现在Utc}）登录成功！`);
                }
            } else {
